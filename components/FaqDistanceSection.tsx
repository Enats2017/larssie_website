...
interface FaqItem {
  faqDistanceItemId: number
  question: string
  answer: string
  images: string[]
}

interface FaqDistanceSectionProps {
  items: FaqItem[]
  distanceName?: string
}

export default function FaqDistanceSection({ items, distanceName }: FaqDistanceSectionProps) {
  ...
  return (
    <section className="py-20 px-4 md:px-10 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center -mb-4">
        <span
          className="font-playlist text-[28px] md:text-[40px] text-sky-500 leading-none"
        >
          Solution
        </span>
      ...
      <h2
        className="font-black text-3xl md:text-4xl text-gray-900 tracking-wide mb-8"
      >
        {distanceName ? `Frequently asked questions — ${distanceName}` : 'Frequently asked questions'}
      </h2>
        ...
        <div>
          <div>
            {filteredItems.map((item, idx) => {
              const isOpen = openId === item.faqDistanceItemId
              return (
                <div
                  key={item.faqDistanceItemId}
                  className={`py-5 ${idx !== 0 ? 'border-t border-gray-200' : ''}`}
                >
                  <button
                    onClick={() => toggle(item.faqDistanceItemId)}
